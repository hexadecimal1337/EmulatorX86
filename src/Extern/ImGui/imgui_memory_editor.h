...
struct MemoryEditor
{
    ...
    void DrawContents(void* mem_data_void, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        ...
        if (ImGui::BeginPopup("OptionsPopup"))
        {
            ...
            if (ImGui::DragInt("##cols", &Cols, 0.2f, 16, 32, "%d cols")) { ContentsWidthChanged = true; if (Cols < 1) Cols = 1; }
            ImGui::Checkbox((const char*)u8"Представить в виде данных", &OptShowDataPreview);
            //ImGui::Checkbox("Show HexII", &OptShowHexII);
            if (ImGui::Checkbox((const char*)u8"Показывать Ascii", &OptShowAscii)) { ContentsWidthChanged = true; }
            ImGui::Checkbox((const char*)u8"Серые нули", &OptGreyOutZeroes);
            ImGui::Checkbox((const char*)u8"Заглавные буквы", &OptUpperCaseHex);
    ...
    void DrawOptionsLine(const Sizes& s, void* mem_data, size_t mem_size, size_t base_display_addr)
    {
        ...
        // Options menu
        if (ImGui::Button((const char*)u8"Параметры"))
            ImGui::OpenPopup("OptionsPopup");
    ...
    void DrawPreviewLine(const Sizes& s, void* mem_data_void, size_t mem_size, size_t base_display_addr)
    {
        ...
        ImGui::Text((const char*)u8"Представить как:");
        ImGui::SameLine();
