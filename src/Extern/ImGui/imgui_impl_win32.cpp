...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport* viewport)
{
    ...
    SetLayeredWindowAttributes(vd->Hwnd, 0, BYTE(255), LWA_ALPHA);
    RECT clientArea{};
    GetClientRect(vd->Hwnd, &clientArea);
    RECT windowArea{};
    GetWindowRect(vd->Hwnd, &windowArea);
    POINT diff{};
    ClientToScreen(vd->Hwnd, &diff);
    ...
    HRGN hRgn = CreateRoundRectRgn(0, 0, viewport->Size.x + 1, viewport->Size.y + 1, style.WindowRounding * 2 , style.WindowRounding * 2);
    SetWindowRgn(vd->Hwnd, hRgn, TRUE);
}
...
{
    // Custom Icon
    static HICON hIcon = LoadIcon(GetModuleHandle(nullptr), MAKEINTRESOURCE(101));
